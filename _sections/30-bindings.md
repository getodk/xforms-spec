...
<a id="fn:randomize" href="#fn:randomize">`randomize(nodeset arg, number seed)`</a> | Shuffles the nodeset argument using [Durstenfeld's version of the Fisher-Yates algorithm](https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle). The optional seed argument performs a (reproducable) seeded shuffle using Fisher Yates with the Park Miller pseudo number generator algorithm.
